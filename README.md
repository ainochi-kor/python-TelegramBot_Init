...
### 5) 소감  <br>
 ...
 처음 이 과제를 만났을 때, 학우들 중에 원래 있던 작업물을 이용하여 서버 작업만 하였던 친구들처럼 저도 만든 봇을 클라우드에 돌릴 수 있도록 방법만 찾으면 될 줄 알았지만 텔레그램 업데이트로 인하여 전체 코딩이 안되는 사고가 생겨, 처음부터 작성을 시작하였고, 수많은 에러를 만났지만 ppt에 수 많은 에러를 적었기 때문에 이 보고서에서는 에러의 내용보다 작업물을 실수 없이 바로 만들 수 있도록 작성하였습니다. 에러의 내용도 추가하면 페이지가 2-3배는 많아질 것으로 예상되어 사용하지 않았습니다.   <br>
